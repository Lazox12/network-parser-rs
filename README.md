...
## Parsing and Calculating Consumed Bytes (`parse_len`)

All structs and enums generated by `network-parser-rs` automatically implement the `NetworkParse` trait. While you can use the standard `TryFrom` trait to parse your types, you can also use the `parse_len` method. This method parses the data and returns both the constructed object and the number of bytes that were consumed from the input slice. This is especially useful for processing streams of multiple packets.

```rust
let data: &[u8] = &[/* your binary data */];
let (frame, bytes_consumed) = EthernetIIFrame::parse_len(data).unwrap();
println!("Parsed frame using {} bytes!", bytes_consumed);
```
